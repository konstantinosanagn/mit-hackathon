...
export function analyzeEditIntent(
  ...
  // const lowerPrompt = ...; // Unused variable
  ...
  for (const [path, fileInfo] of Object.entries(manifest.files)) {
    ...
    for (const word of componentWords) {
      if (fileName.includes(word) || componentName?.includes(word)) {
        console.log(
          `[findComponentFiles] Match found: word="${word}" in file="${path}"`
        );
        files.push(path);
...
function findFeatureInsertionPoints(
  ...
  if (
    ...
    if (locationMatch) {
      ...
      console.log(
        `[findFeatureInsertionPoints] Adding to ${location}, parent files:`,
        parentFiles
      );
    } else {
...
function findComponentByContent(
  ...
  if (files.length === 0) {
    console.log(
      '[findComponentByContent] No files found by content, falling back to component name search'
    );
    return findComponentFiles(prompt, manifest);
  }
