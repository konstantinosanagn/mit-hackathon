...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      '[apply-ai-code] Files to write:',
      parsed.files.map(f => f.path)
    );
    console.log(
      '[apply-ai-code] Existing files:',
    ...
    if (uniquePackages.length > 0) {
      console.log(
        '[apply-ai-code] Installing packages from XML tags and tool calls:',
        uniquePackages
      );

      try {
        const installResponse = await fetch(
          `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/install-packages`,
          {
            method: 'POST',
        ...
        if (installResponse.ok) {
          ...
          console.log(
            '[apply-ai-code] Package installation result:',
            installResult
          );
      ...
      console.log(
        '[apply-ai-code] No packages provided, detecting from generated code...'
      );
      console.log(
        '[apply-ai-code] Number of files to scan:',
        parsed.files.length
      );
      ...
      try {
        ...
        console.log(
          '[apply-ai-code] Package detection response status:',
        ...
        if (packageResponse.ok) {
          ...
          if (
            packageResult.packagesInstalled &&
            packageResult.packagesInstalled.length > 0
          ) {
            results.packagesInstalled = packageResult.packagesInstalled;
            console.log(
              `[apply-ai-code] Installed packages: ${packageResult.packagesInstalled.join(', ')}`
            );
          }

          if (
            packageResult.packagesAlreadyInstalled &&
            packageResult.packagesAlreadyInstalled.length > 0
          ) {
            results.packagesAlreadyInstalled =
              packageResult.packagesAlreadyInstalled;
            console.log(
              `[apply-ai-code] Already installed: ${packageResult.packagesAlreadyInstalled.join(', ')}`
            );
          }

          if (
            packageResult.packagesFailed &&
            packageResult.packagesFailed.length > 0
          ) {
            results.packagesFailed = packageResult.packagesFailed;
            console.error(
              `[apply-ai-code] Failed to install packages: ${packageResult.packagesFailed.join(', ')}`
            );
            results.errors.push(
              `Failed to install packages: ${packageResult.packagesFailed.join(', ')}`
            );
          }
          ...
          if (results.packagesInstalled.length > 0) {
            console.log(
              '[apply-ai-code] Packages were installed, forcing Vite restart...'
            );
            ...
            try {
              ...
              if (restartResponse.ok) {
                ...
                console.log(
                  '[apply-ai-code] Vite restart result:',
                  restartResult.message
                );
              } else {
                console.error(
                  '[apply-ai-code] Failed to restart Vite:',
          ...
          console.error(
            '[apply-ai-code] Package detection/installation failed:',
            await packageResponse.text()
          );
        }
      } catch (error) {
        console.error(
          '[apply-ai-code] Error detecting/installing packages:',
          error
        );
    ...
    const filteredFiles = parsed.files.filter(file => {
      ...
      if (configFiles.includes(fileName)) {
        console.warn(
          `[apply-ai-code] Skipping config file: ${file.path} - already exists in template`
        );
        return false;
      }
      return true;
    ...
    for (const file of filteredFiles) {
      try {
        ...
        console.log(
          `[apply-ai-code] Writing file using E2B files API: ${fullPath}`
        );
        ...
        try {
          ...
          if (global.sandboxState?.fileCache) {
            ...
            console.log(
              `[apply-ai-code] Updated file cache for: ${normalizedPath}`
            );
          }
        } catch (writeError) {
    ...
    const appFileExists =
      global.existingFiles.has('src/App.jsx') ||
    ...
    if (
      ...
      const componentFiles = parsed.files.filter(
        f =>
...
print(f"Auto-generated: {file_path}")
        ...
        results.errors.push(
          `Failed to create App.jsx: ${(error as Error).message}`
        );
      }
      ...
      const indexCssExists =
        global.existingFiles.has('src/index.css') ||
...
if result.stderr:
    ...
    const appFile = parsed.files.find(
      f => f.path === 'src/App.jsx' || f.path === 'App.jsx'
    );
    ...
    if (global.conversationState && results.filesCreated.length > 0) {
      ...
      console.log(
        '[apply-ai-code] Updated conversation state with applied files:',
