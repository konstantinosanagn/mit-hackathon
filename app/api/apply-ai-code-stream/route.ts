...
function parseAIResponse(response: string): ParsedResponse {
      ...
      if (
        ...
        if (!packages.includes(packageName)) {
          ...
          if (
            ...
            console.log(
              `[apply-ai-code-stream] Detected package from imports: ${packageName}`
            );
          }
        }
      }
  ...
  while ((match = fileRegex.exec(response)) !== null) {
    ...
    if (!existing) {
      ...
      console.log(
        `[apply-ai-code-stream] Replacing incomplete ${filePath} with complete version`
      );
    } else if (
      existing.isComplete &&
      hasClosingTag &&
      ...
      console.log(
        `[apply-ai-code-stream] Replacing ${filePath} with longer complete version`
      );
    } else if (
      !existing.isComplete &&
      !hasClosingTag &&
    ...
    if (shouldReplace) {
      ...
      if (
        ...
        console.warn(
          `[apply-ai-code-stream] Warning: ${filePath} contains ellipsis, may be truncated`
        );
  ...
  for (const [path, { content, isComplete }] of fileMap.entries()) {
    if (!isComplete) {
      console.log(
        `[apply-ai-code-stream] Warning: File ${path} appears to be truncated (no closing tag)`
      );
    }
    ...
    for (const pkg of filePackages) {
      if (!sections.packages.includes(pkg)) {
        ...
        console.log(
          `[apply-ai-code-stream] 📦 Package detected from imports: ${pkg}`
        );
      }
    }
  }
  ...
  if (generatedFilesMatch) {
    ...
    console.log(
      `[apply-ai-code-stream] Detected generated files from plain text: ${filesList.join(', ')}`
    );
    ...
    for (const fileName of filesList) {
      ...
      if (fileContentMatch) {
        ...
        if (codeMatch) {
          ...
          console.log(
            `[apply-ai-code-stream] Extracted content for ${filePath}`
          );
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      '[apply-ai-code-stream] Response preview:',
    ...
    if (parsed.files.length > 0) {
      parsed.files.forEach(f => {
        console.log(
          `[apply-ai-code-stream] - ${f.path} (${f.content.length} chars)`
        );
      });
    }
    ...
    if (!sandbox && sandboxId) {
      console.log(
        `[apply-ai-code-stream] Sandbox ${sandboxId} not in this instance, attempting reconnect...`
      );
      ...
      try {
        ...
        console.log(
          `[apply-ai-code-stream] Successfully reconnected to sandbox ${sandboxId}`
        );
        ...
        console.error(
          `[apply-ai-code-stream] Failed to reconnect to sandbox ${sandboxId}:`,
          reconnectError
        );
        ...
        return NextResponse.json({
          ...
          results: {
            ...
            commandsExecuted: [],
            errors: [
              `Sandbox reconnection failed: ${(reconnectError as Error).message}`,
            ],
          },
    ...
    if (!sandbox && !sandboxId) {
      console.log(
        '[apply-ai-code-stream] No sandbox available and no sandboxId provided'
      );
      return NextResponse.json({
        success: false,
    ...
    (async (sandboxInstance, req) => {
      ...
      try {
        ...
        if (allPackages.length !== uniquePackages.length) {
          console.log(
            `[apply-ai-code-stream] Removed ${allPackages.length - uniquePackages.length} duplicate packages`
          );
          console.log(`[apply-ai-code-stream] Original packages:`, allPackages);
          console.log(
            `[apply-ai-code-stream] Deduplicated packages:`,
            uniquePackages
          );
        }
        ...
        if (uniquePackages.length > 0) {
          ...
          try {
            ...
            console.error(
              '[apply-ai-code-stream] Error installing packages:',
              error
            );
            await sendProgress({
              type: 'warning',
