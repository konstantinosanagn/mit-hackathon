  ...
  try {
    ...
    if (!sandbox && sandboxId) {
      ...
      try {
        ...
        console.log(
          `[install-packages] Successfully reconnected to sandbox ${sandboxId}`
        );
      } catch (error) {
        console.error(
          `[install-packages] Failed to reconnect to sandbox:`,
          error
        );
        return NextResponse.json(
          {
...
try:
    ...
    print(f"NEED_INSTALL:{json.dumps(packages_to_check)}")
        ...
        if (
          ...
          for (const line of outputLines) {
            if (line.startsWith('NEED_INSTALL:')) {
              try {
                packagesToInstall = JSON.parse(
                  line.substring('NEED_INSTALL:'.length)
                );
              } catch (e) {
          ...
          console.error(
            '[install-packages] Invalid checkResult structure:',
            checkResult
          );
        ...
        // const packageList = ...; // Unused variable
...
print(f"\\nVerified installed packages: {installed}")
        ...
        const output =
          ...
          installResult?.logs?.stdout?.join('\n') ||
          '';
        const npmOutputLines = output
          .split('\n')
        ...
        const installedMatch = output.match(
