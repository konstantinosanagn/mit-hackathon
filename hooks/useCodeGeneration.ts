...
import { GenerationProgress, CodeApplicationState, ConversationContext } from '@/types/app';
...
export function useCodeGeneration() {
  ...
  const reapplyLastGeneration = useCallback(async (
    ...
    addChatMessage: (content: string, type: any, metadata?: any) => void,
    updateConversationContext: (updates: Partial<ConversationContext>) => void,
    applyGeneratedCode: (code: string, isEdit: boolean, sandboxData: any, addChatMessage: (content: string, type: any, metadata?: any) => void, updateConversationContext: (updates: Partial<ConversationContext>) => void, iframeRef: React.RefObject<HTMLIFrameElement | null>, conversationContext: ConversationContext) => Promise<any>,
    iframeRef: React.RefObject<HTMLIFrameElement | null>
  ) => {
    ...
    await applyGeneratedCode(conversationContext.lastGeneratedCode, isEdit, sandboxData, addChatMessage, updateConversationContext, iframeRef, conversationContext);
  }, []);

  // Memoized generation progress for performance
  const memoizedGenerationProgress = useMemo(() => generationProgress, [
  ...
  const memoizedCodeApplicationState = useMemo(() => codeApplicationState, [
