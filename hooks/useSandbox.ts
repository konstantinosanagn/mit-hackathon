...
export function useSandbox() {
  ...
  const updateStatus = (text: string, active: boolean, type: SandboxStatusType = 'disconnected') => {
  ...
  const checkSandboxStatus = async (silent: boolean = false) => {
    ...
    try {
      ...
      timeoutId = setTimeout(() => {
        if (controller && !isCompleted) {
          try {
            ...
            console.debug('[timeout] Abort error:', error);
          }
        }
      ...
      try {
        ...
        // Only abort if not already completed and not already aborted
        if (controller && !isCompleted && !controller.signal.aborted) {
          controller.abort();
        }
      } catch (cleanupError) {
        console.debug('[cleanup] Error during cleanup:', cleanupError);
      }
    }
  };
    ...
    try {
      ...
      try {
        ...
        if (statusData.active && statusData.healthy && statusData.sandboxData) {
          console.log('[createSandbox] Sandbox already exists and is healthy, using existing sandbox');
        ...
        console.log('[createSandbox] Could not check sandbox status, proceeding with creation');
      }
      ...
      try {
        ...
        timeoutId = setTimeout(() => {
          if (controller && !isCompleted) {
            try {
              ...
              console.debug('[createSandbox] Abort error:', error);
            }
          }
        ...
        if (error.name === 'AbortError') {
          ...
          updateStatus('Network error', false, 'network-error');
          log('Network error - please check your connection and try again.', 'error');
        } else {
        ...
        try {
          ...
          console.debug('[createSandbox] Cleanup error:', cleanupError);
        }
      }
    } finally {
